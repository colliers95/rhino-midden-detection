...
class CNN:
    ...
    def useVGG16(self):
        ...
        self.model = vgg16(weights="VGG16_Weights.DEFAULT").to(
            myDevice
        )  # imports a pretrained vgg16 CNN
        lastLayerIndex = len(self.model.classifier) - 1
        oldFinalLayer = self.model.classifier.__getitem__(lastLayerIndex)
        newFinalLayers = nn.Sequential(
            nn.Linear(in_features=oldFinalLayer.in_features, out_features=2, bias=True),
            nn.Softmax(dim=1),
        )  # changes the output of the last hidden layer to be binary and adds softmax
    ...
    def preprocessData(self):
        ...
        for image in self.images:
            ...
            image = from_numpy(
                (image - 0.5) / 0.5
            ).float()  # rescale pixel values to between -1 and 1
            transformedImages.append(image)

        self.images = transformedImages
        data = list(zip(self.images, self.labels))
        self.trainingDataLength = around(0.8 * self.dataLength).astype(
            int
        )  # use 80% of the data for training
        self.testDataLength = (
            self.dataLength - self.trainingDataLength
        )  # use 20% of the data for testing
        self.trainingData, self.testData = random_split(
            data, [self.trainingDataLength, self.testDataLength]
        )  # splits the data into training and test sets
        self.trainingLoader = DataLoader(
            self.trainingData, batch_size=self.batchSize, shuffle=True, num_workers=1
        )  # batches and shuffles the training data
        self.testLoader = DataLoader(
            self.testData, batch_size=self.batchSize, shuffle=True, num_workers=1
        )  # batches and shuffles the test data
    ...
    def computeClassWeights(self):
        ...
        self.classWeights = [
            numClass1 / self.trainingDataLength,
            1 - numClass1 / self.trainingDataLength,
        ]  # the weights are proportional to the number of points in the other class and sum to 1
        ...
        for epoch in range(self.epochs):  # loop over the dataset multiple times
            ...
            for i, data in enumerate(
                ...
                if i % interval == interval - 1:
                    print(
                        f"Epoch={epoch+1}, Images {self.batchSize*(i+1-interval)}-{self.batchSize*(i+1)}, Loss={runningLoss/interval:.3f}"
    ...
    def activeTrain(self):
        ...
        unlabeledImages = unlabeledData.T[
            0
        ]  # all the training images start out unlabeled
        maxPixelVals = array(
            [amax(unlabeledImages[i]) for i in range(len(unlabeledImages))]
        )  # array of maximum pixel value in each image
        labelingBudget = len(
            self.trainingData
        )  # number of images we are willing to provide labels for
        e = 0.01

        def brightestIndices(threshold):
            return sorted(range(len(maxPixelVals)), key=lambda sub: maxPixelVals[sub])[
                -threshold * len(maxPixelVals) :
            ]
        ...
        newTrainingImageIndices = sorted(
            range(len(maxPixelVals)), key=lambda sub: maxPixelVals[sub]
        )[-self.batchSize :]
        trainingLoader = take(
            unlabeledData, newTrainingImageIndices
        )  # select the images corresponding to the above indices
        ...
        def removeFromUnlabeledImgs():
            for index in sorted(newTrainingImageIndices, reverse=True):
                ...
                del unlabeledImages[
                    index
                ]  # remove image that will be used for training from the unlabeled set
                del maxPixelVals[
                    index
                ]  # remove index that will be used for training from the list of max pixel values
        ...
        while labelingBudget > 0:
            ...
            testImageIndices = sorted(
                range(len(maxPixelVals)), key=lambda sub: maxPixelVals[sub]
            )[-5 * self.batchSize :]
            with no_grad():
                sigmoidOutput = self.model(take(unlabeledImages, testImageIndices)).data
            newTrainingImageIndices = sorted(
                range(len(sigmoidOutput)), key=lambda sub: sigmoidOutput[sub]
            )[-self.batchSize :]
            trainingLoader = append(
                trainingLoader, take(unlabeledData, newTrainingImageIndices)
            )
            removeFromUnlabeledImgs()
    ...
    def test(self):
        ...
        with no_grad():  # since we're not training, we don't need to calculate the gradients for our outputs
            for i, data in enumerate(self.testLoader):
                ...
                _, predicted = max(
                    outputs.data, 1
                )  # predicted is a vector with batchSize elements corresponding to the index of the most likely class of each image in the batch
                ...
                class0Correct += (
                    (take(predicted, class0Indices) == take(labels, class0Indices))
                    .sum()
                    .item()
                )
                ...
                class1Correct += (
                    (take(predicted, class1Indices) == take(labels, class1Indices))
                    .sum()
                    .item()
                )
        ...
        print(
            f"Accuracy of the neural network on the {self.batchSize*(i+1)} test images = {self.accuracy}%"
        )
...
# ThermalCNN = CNN(
#     "Data/TrainingImagesThermal.npy",
#     "Data/TrainingLabelsThermal.npy",
#     batchSize=8,
#     epochs=1,
#     learningRate=0.0001,
#     learningMode="passive",
