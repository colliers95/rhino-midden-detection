...
class DataEngineering:
    """Preprocess midden thermal and RGB into NN-usable or human-viewable format"""

    def __init__(self, orthomosaicPath: str, csvPath: str, imageryType: str):
        ...
        self.trainingLabelMatrices = (
            []
        )  # list of (interval,interval) arrays cropped from the full array of midden locations in the orthomosaic
        self.trainingLabels = []
        self.trainingData = []
    ...
    def getPixelValuesFromTiff(self):
        ...
        for band in range(numBands):
            ...
            imageData[:, :, band] = data.ReadAsArray(
                0, 0, numCols, numRows
            )  # (x offset, y offset, x size, y size)
        if self.imageryType == "Thermal":
            self.orthomosaic = imageData[
                :, :, 3
            ]  # extracting fourth band, which corresponds to temperature
            orthomosaicMin = amin(
                ma.masked_less(self.orthomosaic, 2000)
            )  # 7638 = min pixel value in orthomosaic
            self.orthomosaic = ma.masked_less(
                self.orthomosaic - orthomosaicMin, 0
            ).filled(
                0
            )  # shift the pixel values such that the min of the orthomosaic is 0 and set the background pixels to 0
            self.orthomosaic = (
                (255 / amax(self.orthomosaic)) * self.orthomosaic
            ).astype(
                "uint8"
            ...
            (
                ...
                self.startRow,
                self.endRow,
            ) = self.cropArray(self.orthomosaic)
            array1 = zeros(
                (4000 - self.orthomosaic.shape[0], self.orthomosaic.shape[1])
            )
            self.orthomosaic = vstack((self.orthomosaic, array1))
            array2 = zeros(
                (self.orthomosaic.shape[0], 3400 - self.orthomosaic.shape[1])
            )
            self.orthomosaic = hstack((self.orthomosaic, array2))
        elif self.imageryType == "RGB":
            self.orthomosaic = imageData.astype(uint8)
            print(
                self.imageryType + " orthomosaic shape: " + str(self.orthomosaic.shape)
            )
            indices = []
            for band in range(self.orthomosaic.shape[-1]):
                print(self.orthomosaic[:, :, band].shape)
                _, startCol, endCol, startRow, endRow = self.cropArray(
                    self.orthomosaic[:, :, band]
                )
            ...
            for band in range(self.orthomosaic.shape[-1]):
                self.orthomosaic[:, :, band] = self.orthomosaic[:, :, band][
                    self.startRow : self.endRow, self.startCol : self.endCol
                ]

            array1 = zeros(
                (4000 * 135 / 11 - self.orthomosaic.shape[0], self.orthomosaic.shape[1])
            )
            self.orthomosaic = vstack((self.orthomosaic, array1))
            array2 = zeros(
                (self.orthomosaic.shape[0], 3400 * 135 / 11 - self.orthomosaic.shape[1])
            )
            self.orthomosaic = hstack((self.orthomosaic, array2))
        print(self.imageryType + " orthomosaic shape: " + str(self.orthomosaic.shape))

    def plotImage(self, imageData, title="Figure"):
        """Plot image from numeric image data"""
    ...
    def getMiddenLocs(self, csvPath):
        """Map a list of hand-located middens onto the cropped images"""
        ...
        self.middenCoords.T[0] = (
            self.middenCoords.T[0] - xOrigin
        ) / pixelWidth - self.startCol  # in pixels
        self.middenCoords.T[1] = (
            self.middenCoords.T[1] - yOrigin
        ) / pixelHeight - self.startRow  # in pixels
        self.middenCoords = around(self.middenCoords).astype(int)
        self.middenLocsInOrthomosaic = zeros(
            (self.orthomosaic.shape[0], self.orthomosaic.shape[1])
        ).astype(int)
    ...
    def generateTrainingData(self):
        ...
        while top < int(self.orthomosaic.shape[1]):
            ...
            while right < int(self.orthomosaic.shape[0]):
                self.trainingImages.append(
                    self.orthomosaic[int(bottom) : int(top), int(left) : int(right)]
                )
                self.trainingLabelMatrices.append(
                    self.middenLocsInOrthomosaic[
        ...
        for i in flip(arange(len(self.trainingImages))):
            if all(self.trainingImages[i] == 0):
                ...
                del self.trainingLabelMatrices[
                    i
                ]  # remove label matrices whose corresponding images have all zeros
        for arr in self.trainingLabelMatrices:
            if arr.shape != (40, 40):
                print("wrong shape")
        self.trainingLabels = sum(
            sum(self.trainingLabelMatrices, axis=1), axis=1
        )  # collapses each label matrix to 1 if there's a 1 in the matrix or 0 otherwise
        print(
            self.imageryType
            + " sum of training labels "
            + str(sum(self.trainingLabels))
        )
        self.trainingData = array(
            list(zip(self.trainingImages, self.trainingLabels)), dtype=object
        )  # pairs each training image with its label
    ...
    def showMiddensOnImage(self):
        ...
        for index in sample(
            middenIndices.tolist(), 5
        ):  # look at 10 random images with middens
...
Data = DataEngineering(
    orthomosaicPath="Tiffs/Firestorm3" + argv[1] + ".tif",
    csvPath="Data/MiddenLocations.csv",
    imageryType=argv[1],
)
